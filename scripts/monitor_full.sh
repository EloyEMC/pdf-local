#!/bin/bash
# Monitor completo - VERSIÓN SIMPLIFICADA Y ROBUSTA
...
while true; do
    ...
    # Estado de Ollama
    ollama_status=$(ollama ps 2>/dev/null | tail -n +2)
    ollama_running=$(ps aux | grep "ollama runner" | grep -v grep | wc -l | tr -d ' ')
    ...
    printf "\n"
    printf "════════════════════════════════════════════════════════════════\n"
    printf "      🔄 MONITOR COMPLETO DE PROCESAMIENTO                     \n"
    printf "════════════════════════════════════════════════════════════════\n"
    printf "\n"
    ...
    if [ "$process_running" -gt 0 ]; then
        printf "│ Python process_to_json.py: Activo ✅                    │\n"
    else
        printf "│ Python process_to_json.py: Inactivo ❌                  │\n"
    fi
    if [ "$ollama_running" -gt 0 ]; then
        printf "│ Ollama runner: Activo ✅                                │\n"
    else
        printf "│ Ollama runner: Inactivo ❌                              │\n"
    fi
    printf "└──────────────────────────────────────────────────────────┘\n"
    printf "\n"
    ...
    if [ -n "$ollama_status" ]; then
        printf "🤖 MODELO OLLAMA ACTIVO\n"
        printf "┌──────────────────────────────────────────────────────────┐\n"
        echo "$ollama_status" | head -1 | while IFS= read -r line; do
            printf "│ %-58s│\n" "$line"
        done
        printf "└──────────────────────────────────────────────────────────┘\n"
        printf "\n"
    ...
    if [ "$process_running" -eq 0 ]; then
        printf "⚠️  ADVERTENCIA: El proceso Python NO está corriendo\n"
        printf "\n"
    fi

    if [ "$ollama_running" -eq 0 ]; then
        printf "⚠️  ADVERTENCIA: Ollama NO está corriendo\n"
        printf "\n"
    fi
